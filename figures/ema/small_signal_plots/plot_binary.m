...
keepAspRatio = true;
axislables = ['t','V'];
axisOverlap = [0 .14 0 .14];  % [-] x-axis normalized
...
%%% FUNCTION
x = 0:0.05:5;
y = (square(sin((x)/1*pi))+1)/2*(1/3)+(2/3);
z = (square(sin((x/2)/1*pi))+1)/2*(1/3)+0;
...
y = y * iYdif;
z = z * iYdif;
oYlim = outerYlim([min(z), min(z) + oYdif - sum([axisOverlap(3:4);frameSep(3:4)],'all') * oXdif], ...
...
hold on
plot(x,y,'LineWidth',plotLineWidth,'Color',pttColors{6});
plot(x,z,'LineWidth',plotLineWidth,'Color',pttColors{4});
for i = 1:2
    ah(i) = annotation('arrow','HeadStyle','plain', ...
        'LineWidth',axisLineWidth,'HeadLength',12*oXdif*arrowLength, ...
        'HeadWidth',2*12*oXdif*arrowLength*tand(arrowAng/2));
    ...
    if i == 1
        ...
        set(ah(i), 'position',[posmat(1:2),diff(posmat,1,2).']);
        set(lh(i), 'position',[posmat(3:4)+[0.1,0.1]*oXdif,0.1,0.1]);
    end
end
h = get(gca, 'Children');
set(gca, 'Children', [h(3),h(5),h(1),h(2),h(4),h(6)]);
