...
% function dependent
y(:,:,1) = y(:,:,1)*oYdif/14;
oYlim = outerYlim([min(y(:,:,1)), min(y(:,:,1)) + oYdif - sum([axisOverlap(3:4);frameSep(3:4)],'all') * oXdif], ...
...
stem(x(1:5:end),y(1:5:end,:,1),'filled','LineWidth',plotLineWidth, ...
    'MarkerSize',0.9,'Color',pttColors{4});
for i = 1:2
    ah(i) = annotation('arrow','HeadStyle','plain', ...
        'LineWidth',axisLineWidth,'HeadLength',3.6*oXdif*arrowLength, ...
        'HeadWidth',2*3.6*oXdif*arrowLength*tand(arrowAng/2));
