...
%%% Parameters
saveplots = true;
showpreview = false;    % not very accuarte preview (open emf to see result)
...
width = 15;        % [cm]
height = 15;       % [cm] iff ~keepAspectratio
...
plot(x,z,'--','LineWidth',plotLineWidth,'Color',pttColors{7});
stem(xp,yp,'Linewidth',plotLineWidth,'Color',pttColors{1});
for i = 1:2
    ...
    if i == 1
        ...
        set(lh(i), 'position',[posmat(3:4)+[-0.05, 0.08]*oXdif,0.1,0.1]);
    else
        posmat = [zeros(1,2); (oYlim + [1 -1].*frameSep(3:4)*oXdif)];
        set(ah(i), 'position',[posmat(1:2),diff(posmat,1,2).']);
        set(lh(i), 'position',[posmat(3:4)+[0.03,-0.02]*oXdif,0.1,0.1]);
    end
end
h = get(gca, 'Children');
set(gca, 'Children', [h(4),h(6),h(1),h(2),h(3),h(5),h(7)]);
