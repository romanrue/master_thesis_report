...
try:
    ...
    import numpy as np
    from control import ss, impulse_response
...
plotLineWidth=0.8       # [pt] line width of plotted stems
plotMarkerSize=1        # [pt] marker size of plotted markers
...
def plot(x,y,step=2,ax=None):
    if ax is None:
        ax = plt.gca()
    # Plots
    markerpts, stemline, _ = ax.stem(x[::step],y[::step],basefmt=' ')
    plt.setp(stemline, linewidth=plotLineWidth)
    plt.setp(markerpts, markersize=plotMarkerSize)
